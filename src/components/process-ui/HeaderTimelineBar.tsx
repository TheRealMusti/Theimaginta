...
export function HeaderTimelineBar() {
    ...
    return (
        <div className="w-full relative overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-4 md:pb-0">
            <GlassPane 
                padding="0" 
                className="h-[56px] min-w-[700px] w-full rounded-pill flex items-center border-white/[0.06] bg-white/[0.01]"
            >
                {PHASES.map((phase, index) => {
                    ...
                    return (
                        ...
                        >
                            {/* Number */}
                            <Meta className={`transition-colors duration-300 ${isHovered ? 'text-accent-base' : 'text-white/[0.30]'}`}>
                                {phase.num}
                            </Meta>
                            ...
                            {index < PHASES.length - 1 && (
                                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[0.5px] h-[24px] bg-white/[0.06]" />
                            )}
                        </div>
                    );
                })}
