...
export function ProcessJourney({ children }: { children: React.ReactNode }) {
 ...
 <div className="absolute top-[20px] bottom-0 left-[24px] xl:left-[calc(50vw-600px+80px)] w-[1px] bg-[#F5F2ED]/[0.04] z-0" />
...
export function PhaseNode({ className = "" }: PhaseNodeProps) {
 ...
 <div 
 ref={ref}
 className={`absolute left-[19px] xl:left-[calc(50vw-600px+75.5px)] w-[10px] h-[10px] rounded-full transition-all duration-600 ease ${isInView ? 'bg-accent-base/30 border-accent-base/20 ' : 'bg-[#F5F2ED]/[0.06] border-[#F5F2ED]/[0.08]'} border-[0.5px] z-10 ${className}`} 
 />
 );
}
