...
export function TimelineSummary() {
 ...
 <FadeUp>
 <h2 className="font-sans text-[32px] md:text-[40px] font-bold tracking-[-0.03em] text-[#F5F2ED]">The whole picture.</h2>
 <p className="font-sans text-[16px] font-normal text-[#F5F2ED]/[0.72] mt-[16px] max-w-[400px]">
 ...
 <FadeUp delay={0.2} className="hidden md:flex w-full h-[80px] rounded-pill bg-[#F5F2ED]/[0.02] border border-[#F5F2ED]/[0.06] p-[4px]">
 {PHASES.map((phase, i) => {
 const isHovered = hoveredIndex === i;
 return (
 <div 
 key={i}
 className={`relative h-full rounded-pill transition-all duration-300 flex items-center justify-center cursor-default ${isHovered ? 'bg-accent-base/[0.1] border border-accent-base/[0.2]' : 'hover:bg-[#F5F2ED]/[0.04]'}`}
 ...
 <span className="font-sans text-[12px] font-medium text-[#F5F2ED] whitespace-nowrap">{phase.name}</span>
 </div>
 <div className={`transition-opacity duration-300 ${!isHovered ? 'opacity-100' : 'opacity-0'}`}>
 <Meta className="text-[#F5F2ED]/[0.3]">{phase.num}</Meta>
 </div>
 </div>
 );
 })}
 ...
 {PHASES.map((phase, i) => (
 <div key={i} className="w-full p-[20px] rounded-[12px] bg-[#F5F2ED]/[0.02] border border-[#F5F2ED]/[0.06] flex items-center justify-between">
 <div className="flex items-center gap-[16px]">
 <Meta className="text-[#F5F2ED]/[0.3]">{phase.num}</Meta>
 <span className="font-sans text-[15px] font-medium text-[#F5F2ED]">{phase.name}</span>
 </div>
